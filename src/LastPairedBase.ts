...
/**
 * The last paired base in a target structure.
 */
export class LastPairedBase<Nucleobase> {
  ...
  get(): Nucleobase | never {
    ...
    let pairedBaseIndices = pairedBases.map(b => seq.indexOf(b));

    if (max(pairedBaseIndices) < 0) {
      throw new Error('No paired bases present in sequence.');
    }

    return seq[max(pairedBaseIndices)];
  }
}
